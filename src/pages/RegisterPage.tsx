...
export const RegisterPage = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Wait, UserProfileContext handles default profile on state change
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-6 font-body" dir="rtl">
      <div className="w-full max-w-md space-y-6">
        <div className="flex flex-col items-center gap-3 text-center mb-2">
          ...
          <p className="text-textMuted font-semibold text-sm">ابدأ بإنشاء الـ QR الذكي الخاص بك بدقائق</p>
        </div>

        <Card className="border-border shadow-card bg-white rounded-2xl">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-textDark text-right">إنشاء حساب جديد</CardTitle>
          </CardHeader>
          <CardContent className="p-6 pt-0">
            <form onSubmit={handleRegister} className="space-y-4 text-right">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-textDark/80">الاسم الكامل</label>
                <Input 
                  type="text" 
                  value={name} 
